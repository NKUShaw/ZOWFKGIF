...
class FilteredTrainDataLoader(TrainDataLoader):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)

    def filter_entities_and_triples(self, num_entities_to_remove):
        entities_to_remove = set(random.sample(range(self.entTotal), num_entities_to_remove))
        filtered_triples = []
        with open(self.tri_file, 'r') as f:
            next(f)
            for line in f:
                head, tail, rel = map(int, line.strip().split())
                if head not in entities_to_remove and tail not in entities_to_remove:
                    filtered_triples.append((head, tail, rel))

        self.triples = filtered_triples
        self.tripleTotal = len(filtered_triples)

    def read(self, num_entities_to_remove):
        self.filter_entities_and_triples(num_entities_to_remove)
        if self.batch_size is None:
            self.batch_size = self.tripleTotal // self.nbatches
        if self.nbatches is None or self.nbatches == 0:
            self.nbatches = self.tripleTotal // self.batch_size
        self.batch_seq_size = self.batch_size * (1 + self.negative_ent + self.negative_rel)
        self.batch_h = np.zeros(self.batch_seq_size, dtype=np.int64)
        self.batch_t = np.zeros(self.batch_seq_size, dtype=np.int64)
